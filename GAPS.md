...
- ⏭️ **Cross-market pooled 학습 기각(2026-06-23)**: US+KR 합쳐 학습 → KR IC 0.0109(=baseline, 이득0)/US IC 0.0356→0.0141(희석 손해). 단일시드 KR 0.0133은 노이즈. 비대칭전이로 약한시장 보강 기대했으나 KR 자기모델이 이미 동급.
- ⏭️ **Short-volume/공매도 피처 기각(2026-06-23)**: 17.9M행 피처화(sv_ratio/chg/z, `augment_shortvol.py`) → US IC 0.0356→0.0329(미세↓). 일별 short VOLUME=MM헤지 노이즈(진짜 anomaly는 short INTEREST=DB에 없음), 트리가 이미 가격서 포착.
- 🔴 **PEAD/어닝서프라이즈 — 역사데이터 없음**: earnings_surprise 2018-2024 단 46행/2종목(전체 1538행은 2025-26 최근). 뉴스 I축과 동일 — **대체데이터 전부 역사 없음**(최근 수집 시작). 백필 후에야.
...
- 📉 (구) 프록시 결과는 참고용: 프록시로는 알파가 벤치 하회했으나 프로덕션 레시피로 반전. 'IC≠top-decile수익' 교훈은 유효하나 5레버가 top-decile 수익까지 끌어올림.
- 🟢 **타이밍 게이트가 값을 더함(핵심 통합가설 지지)** — 완성된 2×2 매트릭스(US 20회, walk-forward, Sharpe 기준):
  - 벤치(알파✗기술✗) 0.59 · **알파단독(알파✓기술✗) 0.43**(벤치 하회, 프록시 엣지 미묘) · **기술단독(알파✗기술✓) 0.67**(총+37.1%≈벤치, 투자62%로 위험조정↑) · 병합-cash 0.67 · **병합-집중(알파✓기술✓) 0.72, 총+50.6%**(벤치·단일요인 모두 상회).
  - **시너지 확인**: 병합-집중 Sharpe 0.72 > max(알파 0.43, 기술 0.67). 알파가 종목풀을, 기술이 타이밍을 담당해 **둘 다 단독보다 나음**. 단서: 간이 프록시·20회·단일시장, 확정 아님. paper가 최종.
