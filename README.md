...
### 통합 전략 (선정→실행)
횡단면 알파가 **무엇을** 살지(시황·선정) 정하고, 기술적 지표가 **언제** 살지(실행 타이밍)만 결정하는 2계층 파이프라인. core-kr/us 계정. (설계: `DESIGN_INTEGRATED.md`, 아키텍처: `ARCHITECTURE.md`)
- **시황 읽기** — 시장별 레짐 + 시장폭(breadth) + 선정 확신도 → 권장 투자 비중(목표 노출). "지금 얼마나 투자할까"
- **선정 바스켓** — 알파가 고른 종목(능동 인덱스, 균등 비중) + 예상수익/목표가/밴드 + 전일 대비 NEW/보유/제외
- **실행 타이밍** — 바스켓 종목별 기술적 진입/청산 판정(BUY/대기/매도/거절) + 기술점수 + 타이밍 사유. 기술적은 타이밍만 판단(선정 거부 불가)
