...
### 의미 / 다음
- 정직한 cross-regime 알파 **~0(rank) → ~+10%/년(mn)**. 두 자릿수 목표 가시화.
- 방법론 교훈: **엄밀 평가(재선택+다중시드)가 필수**. 좋아 보이는 단일 결과는 대부분 artifact.
- 다음: **mn 라벨 채택** → 프로덕션 타깃 교체 + 그 위에서 레버 계속(피처선택 강건화/중립화/앙상블/Wave4/튜닝).
