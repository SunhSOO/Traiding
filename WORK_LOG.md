...
**LTR(learning-to-rank, B) 기각:** `--ltr-test`(회귀 vs LGBMRanker lambdarank, alpha-only WF).
- US: 회귀 Sharpe 0.75/IC +0.0145 vs 랭커 0.64/**IC −0.0050**. KR: 회귀 0.33/+0.0232 vs 랭커 0.48/**IC −0.0150**.
- **핵심: 랭커는 총수익 높은 곳(KR)서도 IC 음수** → 알파캠페인 교훈(IC≈0/음수 + 큰수익 = 팩터틸트, 선택스킬 아님; ridge 가짜승리 패턴)에 해당. **IC 규율로 기각.** 회귀(일관 양수 IC) 유지. [[alpha-campaign-result]]

**숏볼륨(C) 종결:** short_ratio/short_squeeze 피처는 캐시에 있으나 알파 top-50 미선택(중요도 순위 149~441/464). 숨은 알파 아님 — 알파포화 재확인.

> 이번 비-저장공간-차단 미탐축 종합: **스톱=강력 채택(리스크 최대 개선), LTR=기각(음수 IC 팩터틸트), 숏볼륨=종결(약함).**

**F/T/I 복합엔진↔통합 블렌딩(마지막 축) 종결 — 구조적 중복:** ML 알파가 **펀더멘털 피처 69개 이미 사용**(pe_ttm·roe_q·roa_q·debt_equity·piotroski_score·altman_z·insider_net 등 = 복합 F모듈 입력 전부). 복합 T모듈=이미 실행 타이밍 게이트. I모듈(뉴스)만 차별점인데 저장공간 차단. → 규칙기반 복합을 ML(같은데이터 최적학습)에 블렌딩은 F·T 중복/I 차단이라 무거운 백테스트 불요, 저가치 종결.

> **비-저장공간 가능성 공간 소진 확정.** 남은 미탐은 전부 **데이터·저장공간 차단**: (1)정보/뉴스축 10년백필(~5TB), (2)KR고유 방향신호(외국인수급·원화·중국 = 신규 인제스트). 둘 다 자원 문제. 코드/백테스트로 짜낼 여지 소진.
...
### ⚠️ 엄격 forward-HOLDOUT — 위 결론 정정 (2026-07-06, honesty)

근거: `scratchpad/holdout_test.py`. **모델이 학습에 전혀 안 쓴 데이터(2025-06~2026-05)로 검정** — 2018-2023 학습 → 2025-2026 예측, 사이 **1.5년 gap**(누출 없음). rank·mn 두 라벨 모두:

| 미지 2025-26 (target=mn) | KR baseline | KR new | US baseline | US new |
|---|---|---|---|---|
| rank-IC(스킬) | +0.0606 | +0.0571 | -0.0032 | -0.0016 |
| top-decile 21d ret | +13.22% | +13.02% | +1.82% | +1.94% |

- **신규피처는 미지 최근 구간에서 중립** — IC가 baseline과 노이즈 내 동일(KR은 baseline 근소우위, US는 둘 다 ~0). rank 라벨도 동일 결론(KR IC +0.058→+0.030).
- **∴ 위 walk-forward(2018-24)의 KR 개선(+0.03→+0.12)은 미지 2025-26으로 일반화되지 않음.** 그 이득은 구간특이적(2018-24 레짐)이었을 가능성. **"OOS 실측 개선/검증된 승자"는 과claim이었고 정정한다: in-sample 개선 O, 엄격 out-of-period 중립.**
- 신규피처는 **무해**(Sharpe 3/4서 근소↑, IC 노이즈내). 배포 유지하되 **이득 미확인(status=proposed)**. US는 양쪽 IC≈0 = 최근 US 횡단면 스킬 사실상 없음(수익=팩터틸트, 가짜승리 패턴).
- 한계: holdout은 2024 gap으로 stale모델 검정 → 실배포(최근 재학습 모델)보다 harsh할 수 있음. 그래도 "신규피처가 일반화 이득을 준다"는 근거는 미확인.

> **정정된 결론: 신규 무료데이터는 in-sample(2018-24)선 KR을 개선했으나, 엄격한 미지-데이터(2025-26) 검정에선 중립 — 일반화 이득 미확인. 무해하므로 배포는 유지, 그러나 "검증된 알파"로 표기하지 않는다.** ([[feedback_dev_principles]] 정직 우선, 가짜승리 회피)

---

## 2026-07-07 (화) — 자동매매기 end-to-end 미지-데이터 검정 + 3-과제(전체)

근거: `scripts/run_integrated_job.py`(라이브 실행), `backtest_integrated.py --production --with-timing`, 프로파일링, `scratchpad/{holdout_test,fresh_test,debug_helpers,verify_fundv2}.py`

**① 작동(works)**: `run_integrated_job` end-to-end 정상 — 선정→타이밍→리스크→페이퍼체결→계좌, **errors=0** 양시장(KR 20포지션 999,917KRW / US 20포지션 973USD). 페이퍼트레이딩 잡 `integrated.daily`(23:00)·`decisions.daily`(22:30) **enabled** → 매일 자동 실행(관측은 달력시간 경과 필요).

**② 성능(walk-forward OOS, 전체 기계=선정+기술적타이밍)**:
| 시장 | 벤치(EW유니버스) | 우리 기계 | 초과 | 타이밍효과 |
|------|------------------|-----------|------|-----------|
| US | +36.7%(Sh 0.59) | **+61.6%(Sh 0.68)** | +25.0%p | 집중 +23.4%p |
| KR | **-17.1%**(Sh -0.11) | **+1.0%(Sh 0.12)** | +18.0%p | 집중 +16.3%p |
- KR: 시장 -17% 하락장에서 +1.0% 방어. 타이밍 게이트 양시장 도움.

**엄격 미지-데이터(2025-26 holdout, ≤2024 학습)**: 앞 절대로 **신규피처 중립**. 추가로 **US 선정 IC≈0**(스킬 실종).

**② 파이프라인 최적화 시도 → 안전 롤백**: 6~7월 라이브 시뮬을 위해 빌드 가속 시도. 프로파일 결과 병목=`features_fundamental_v2`(66%)+`features_advanced`(18%)의 per-date O(dates×concepts) 루프. 캐시화 구현 후 검증서 **fundamental_v2의 tie-break 비결정성 발견**: 같은 `as_of_ts`(정정공시·동일일 복수공시) 동률 시 pandas 기본 quicksort(unstable)로 **원본이 이미 임의·쿼리일마다 변동**. 캐시로 원본과 정확일치 불가 + 결정론화하면 펀더멘털 피처 변경→캐시재빌드+재학습 캐스케이드. **모델 안정성 우선 → 최적화 롤백(git checkout), 모델 원상 보존.** 후속과제로 남김(결정론적 tie-break=max as_of_ts, then max period_end + 재학습). **∴ 6~7월 라이브 시뮬은 빌드속도로 미완**(로컬 환경 한계, 3종목 프로파일 257s).

**③ US 엣지 약화 진단**: 원인=**저기회 아님**. 횡단면 dispersion(fwd21d std) 연도별: 2018-23 ~6.8-8.3% vs **2025 9.4% / 2026 11.6%(최고)** = 선정 기회는 오히려 증가. 즉 US IC≈0 = **진짜 알파 디케이**(2018-24 학습 관계가 2025-26에 전이 안 됨), 기계적 저분산 아님. **완화책=주간 재학습 `training.weekly`(ON, Sun04:00)로 최근데이터 적응.** 갭: `mlops.drift_check.daily`·`mlops.retrain.weekly` **OFF** → 디케이 자동감지/알림 부재(활성화 권장).
...
### 추가(2026-07-10): 알파 성장 로드맵 + **첫 강건 생존자 = KR EWMA rank 평활**
"알파 키울 다른 방법?" → 4렌즈 발산생성+적대가지치기 로드맵(scratchpad/alpha_growth_wf.js). 두 실험 실행:
...
### 추가(2026-07-10, "모두 다 수행" 완결): B1 미채택 + C4v2 진짜감쇠답 + 데이터갭 완성
- **B1(레짐분할 KR모델) 미채택 — 또 단일-seed 아티팩트**(`var/_analysis/wf_b1_kr_robust.py`). 단일run routed +2.10% vs pooled +1.18%(stress 5/5)로 유망했으나 3-seed서 routed-pooled +0.00/−0.99/−0.52%(2/3 seed 손해, winVsPool 56/25/44%). stress이득은 3-seed 모두 양수나 3/5폴드·1폴드집중, calm손해가 상쇄. → KR EWMA와 동일 패턴. C2(유동성)=KR알파가 mid-테르섬 집중(+1.96%)이나 Sharpe 미개선, 미미.
- **✅ C4v2 진짜 감쇠답**(`var/_analysis/decay_v2_us.py`, US 2018-2026 연속·매폴드 재학습): PRE-2024 top +1.44%/spread +1.29% ↔ **2024-26 top +1.71%/spread +1.88%/80%폴드양수**(분기 8/10, 집중아님), **단 IC≈0(−0.018)**. **US "감쇠"=스테일+rank-IC 착시. 신선모델 top-decile(거래대상) 미붕괴** — 주간재학습이 US살림. US 감쇠-대응 모델 불필요. **드리프트모니터는 rank-IC 아닌 top-decile excess 추적.**
- **✅ 데이터갭 완성**: US(`_bt_period_US_2023-06-01_2026-07-01`, 388k행) + KR(`_bt_period_KR_2023-06-01_2026-07-01`, 248k행). **양시장 2018-2026 연속** → 최근창 재검증·뉴스 I축 즉시테스트 가능.
- **"모두 다" 4방향 완결**: #1 배선=취소(채택레버 없음), #2 갭=완성, #3 로드맵=C2/B1 기각, #4 재검증=EWMA/B1 기각. **모델공간 알파 소진 확정(6레버 반증). 알파성장 유일경로=데이터 프론티어.** (연대기: ALPHA_CAMPAIGN 시점 17)

### 추가(2026-07-13): 뉴스 백필 상태 정정 + A5(오버나이트) 기각 + **유니버스=알파천장 통찰 → 소형주 피벗**
- **뉴스 백필 정직 정정**(`gdelt_history_backfill.py --dry-run`): "진행중/8-10월 자동완성"은 조건부였음. 실상=**7월 BQ 무료쿼터 92% 소진**(month_used 0.924/0.95TB, 청크당 0.35TB→이번달 0개 가능). news min 2024-05 고정(2년치). 8/1 리셋 후 재개, 무료속도 ~3청크/월→**10월경 6년완성**(서버 상시가동 or 수동실행 필요). 더빠르게=유료뿐(원칙위반). KR뉴스는 여전히 벽. [[project_news_history_plan]] 갱신.
- **A5(오버나이트 vs 인트라데이 모멘텀) 기각**(`var/_analysis/wf_a5_overnight.py`): OPEN 필드=모델 미사용 진짜 새정보이나 US 3-seed서 base +1.14%→a5 +1.21%(노이즈), **winVsBase 50%**, A5피처 top-50 선택 0.6/5, 표준 daily-IC 음수(−0.02~0). **모델 레버뿐 아니라 새 가격신호도 대형주선 무효.**
- **🔑 통찰: 알파 천장은 신호가 아니라 유니버스가 정한다.** 6모델레버+A5까지 전부 실패한 근본원인=우리가 **가장 arbitraged된 대형주 풀**(S&P500/NDX100/KOSPI200/KOSDAQ150)에서 낚시. 대형주 횡단면 선정알파는 얇고 이미 가격피처에 흡수됨. **덜 효율적 유니버스(소형주)로 가야 알파 천장 자체가 오름**(C2가 KR mid-유동성 우위로 힌트).
- **→ 다음: 소형주/광범위 유니버스 피벗 테스트**(user 지시). US mid(S&P400)·small(S&P600/R2000)·KR KOSDAQ소형 vs 대형 벤치. 우리가 만든 파이프라인(가격피처+walk-forward top-decile excess 3-seed)으로 "먹히는지" 광범위·면밀 검증. (새 캠페인)

### 추가(2026-07-14): 유니버스 캡스펙트럼 테스트 → **캠페인 첫 강건 긍정: US 소형주에 진짜 IC**
`var/_analysis/{get_universes,universe_alpha_test}.py`(FDR멤버십+yfinance가격, 린 20피처, walk-forward top-decile excess 3-seed, 2018-2026, 각 ~350종목):
| 유니버스 | meanIC | IC H1/H2 | net@cost |
|---|---|---|---|
| US_LARGE | +0.0005 | −0.017/+0.018(반전) | +0.91%@20bp |
| US_MID | +0.012 | −0.016/+0.040 | +0.27%@35bp |
| **US_SMALL** | **+0.040** | **+0.018/+0.061(양half+)** | +0.31%@50bp |
| KR_LARGE | +0.042 | +0.001/+0.081 | +2.59%@20bp |
| KR_MID | +0.040 | +0.025/+0.054(양half+) | +0.66%@35bp |
| KR_SMALL | +0.035 | +0.003/+0.065 | +0.59%@50bp |
- **✅ 가설 확증(US): 선정 IC 캡사다리 내려갈수록 ~80배↑**(대형 0.0005→소형 0.040). **US_SMALL만 두 하위기간 모두 IC 양수** = 유일 강건 US 스킬, 50bps 후 +0.31%. US 대형주 +1.0% excess는 IC≈0/부호반전=**베타틸트**. **우리 US "감쇠"의 진짜 원인=가장 arbitraged된 유니버스 선택.** KR은 전 캡 비효율(0.035-0.042), KR_MID 최안정, 소형화 불필요.
- **정직 유보(배포 전)**: ①생존편향(현재멤버십, 소형 위로편향—단 IC는 수익레벨보다 덜 민감) ②비용(순+0.31% modest) ③린20피처(풀484 다를수). 방향은 강건확증, 배포 매그니튜드는 풀파이프라인+PIT멤버십 필요.
- **∴ 6모델레버 실패 후 첫 강건 긍정. 알파성장 경로=①US 소형주 유니버스 확장(구조) ②뉴스 I축(데이터).** (연대기: ALPHA_CAMPAIGN 시점 18)
- **다음**: US 소형주에 풀 파이프라인(484피처+mn/tb 라벨) 투입 + 생존편향(PIT 멤버십) 해결 + 비용/유동성/실매매 접근성(Alpaca) 평가.

### 정정(2026-07-14, 3단계 검증): **US 소형주=큐레이션/생존편향 아티팩트(사실상 기각), 진짜 신호=KR 중형**
"순서대로 모두 진행" 지시로 3단계 실행:
- **1단계(풀파이프라인 대리=리치 42피처)**: 소형주 IC 미개선(0.040→0.031, 가격피처 포화), **대형주는 −0.019 과적합**. ⇒ 풀파이프라인 가치는 가격 아닌 펀더멘털/정보축(DB인제스트 필요).
- **2단계(생존편향)**: **큐레이션 없는 광역 US(US_BROAD 251종목) IC −0.018**(양 하위기간 음!), 유동성플로어 50%서도 ≈0(+0.0025). **∴ S&P600 IC 0.040 = 지수 committee 선정+생존편향 아티팩트, "소형주 알파" 일반성질 아님.** US 소형주 확장 사실상 기각. (PIT 멤버십 무료 완전보정=불가, CRSP급 유료 필요.)
- **✅ 진짜 신호=KR**: KR_MID는 **큐레이션 없는 순수 marcap 슬라이스인데 IC 0.042**(양 하위기간+/리치피처 유지). US_SMALL(큐레이션 의존)과 결정적 차이. KR 전 캡 비효율=구조적. **알파 홈=KR 중/소형.** 유보: KR도 생존편향(PIT 확인 필요)이나 uncurated 견고성이 US_SMALL보다 강함.
- **3단계(실행) 전제 변경**: 알파가 KR이므로 실행=**KIS(KR)**, Alpaca(US) 아님 — 지금 Alpaca 지으면 반증한 US엣지 실행 짓는 셈. **user에 KIS 리다이렉트 확인 요청.**
- **campaign 순결론**: US 소형주도 아티팩트(세션 패턴 재현). 유일 강건=KR 중형. (연대기: ALPHA_CAMPAIGN 시점 18 정정)

### 추가(2026-07-14, A로 진행): **KR 소형주 알파 확정 — 세션 유일 완전-강건 긍정**
US를 죽인 uncurated 딥테일 교차검증의 KR판 + 비용스윕:
| KR버킷 | IC | IC H1/H2 | gross | net@100bp | net@250bp |
|---|---|---|---|---|---|
| **KR_MICRO(1000-1600)** | **0.046** | **+0.047/+0.045** | +2.61% | +1.73% | +0.40% |
...
- **✅ KR_MICRO=최강·최안정**: US uncurated 딥테일 IC −0.018 붕괴 ↔ **KR 딥테일 IC 0.046, 양 하위기간 거의 동일(전체 최강 시기안정), 250bp서도 net +0.40%.** KR=구조적 비효율 확정(큐레이션 아님, US 죽인 검증 통과). **3-seed·양 하위기간·리치피처·uncurated딥테일·비용스윕 전부 통과=세션 유일 완전-강건 긍정.**
...
### 추가(2026-07-20, "진행해봐"=H42 프로덕션 반영 시도): **미전이 → train_production 무변경 (시점 23)**
배포 전 검증(`wf_prod_horizon_kr.py`, 풀 478피처·프로덕션 유니버스·연속 2018-2026, 42d=pykrx close 조인 91%):
| 호라이즌 | folds | ic | net_ANN | best-2 | bear |
|---|---|---|---|---|---|
| H21(base) | 75 | 0.0089 | +12.3% | +1.09% | **+0.35%** |
...
### 추가(2026-07-24, 사용자 "모든 검증 했나? 놓친 것 없나?"): 선제 gap 검증 — "순수 비유동" 과장 적발 (시점 32)
안 물어봐도 미검증 gap 6개 나열 후 위험도순 실행:
- **478피처 전수 누출 스캔**(`verify_gaps.py`): KR/US 모두 |IC|>0.10 **0개** — 숨은 누출 없음, **시점24-26 프로덕션 재감사 오염 안 됨 검증**(vol_adj_ret_21d는 피처셋 밖이었음).
- **US 스플릿조정**: US_LARGE clean(>80%이동 0/347)·**US_MID(5, CHRD 257x)·US_SMALL(8) 미조정 오염**(이미 caveat, 추가확인). US_SMALL Sharpe 2.02는 오염가능.
- **생존편향 광범위**: US_LARGE 5%(최소·기관픽 견고)·KR_MID 16%·KR_MICRO 23%(최대·최저신뢰).
...
### 추가(2026-07-24, 사용자 "모두 수행해봐"): 잔여 ~70체크 4배치 전수 실행 (시점 34)
CHECKLIST 잔여 검증을 4배치로 전수 실행(`robustness_batch1/2/3/3b.py`). **결과 대부분이 엣지를 강화**했고, 이전 "저용량" 우려를 자발적 de-confound로 **반전**시킴.
- **batch1 신호/교란 강건성**: **다변량 Fama-MacBeth**(fwd~ILLIQ+SIZE+VOL+R5+R21 동시통제) ILLIQ 한계 **t=1.74(유의미달)**·VOL t=2.49 최강 → 헤드라인은 size·vol·reversal known-factor 블렌드. amihud spec(median +1.88·log +1.83) 강건·1/price중립 +0.87·vol중립 +1.82·**전팩터중립 잔차 +0.73%**. decile top5/10/20 단조(+1.96/+1.94/+1.34). ex-COVID +1.95(불변). **위상 6개 평균 +1.74±0.18%**. **연도별 2019-25 전부 양(+)**(최악 2022 bear도 +0.54, 최고 2023 +3.28).
- **batch2 다중검정 rigor**: 후보별 DSR(N=135) — **KR_LARGE 97.1%·KR_MID 94.9%·US_LARGE 85.5% 생존**, KR_MICRO 13.5%·KR_SMALL 31.7% 탈락. **FWER: KR_LARGE·KR_MID는 family N=135서 Bonferroni(최보수)·Holm까지 생존**(US는 BH-FDR만). 선택 haircut: KR_LARGE DSR N=400→93.9%·N=1000(초보수)→89.9%. **PBO/CSCV=0%**(IS-best amihud window가 OOS 중앙값 상회 100%=window-과적합 없음). → "알파는 다중검정 운"을 직접 반박.
- **batch3 현실성/기전**: **marcap-clean 중립화**(shares=NET_INCOME/EPS_BASIC) — KR_LARGE 잔차 **+0.77%(t=1.48, 유의미달)=대부분 size틸트 확정**(시점33 결론 재확인); KR_MID +1.89%(t=2.11)이나 **share커버 28% 부분표본=저검정력**(과claim 금지). **★가중: ADV-weight(더 유동적 이름 집중)가 엣지를 오히려 강화**(KR_LARGE equal +1.94→ADV +2.52%·KR_MID +1.58→+2.56%, t>4). limit-move(형성일 |ret|≥29%) 제외해도 불변(+1.94/+1.65). overlap-2+반비용 +2.09/+1.78.
- **batch3b ADV개선 de-confound**(과claim 방지 자발적): 동일-ADV가중 벤치 대비 clean 초과 **KR_LARGE +2.48%(t=3.12)·KR_MID +3.74%(t=4.49)=진짜 selection**. **illiquid-decile 中 유동적 절반이 최소유동 절반보다 강함**(KR_LARGE +2.42 vs +1.43·KR_MID +2.30 vs +0.82) → **tiny-name/bid-ask-bounce 아티팩트 아님·realizable at scale**(#5.2 부분해소).
- **∴ 시점34 — 정직한 최종 종합**: KR_LARGE/MID ILLIQ 엣지는 **(a)** 체결지연·spec·decile·COVID·위상·전연도·다중검정(Bonferroni N=135)·PBO에 **전부 견고 = 진짜**, **(b)** 그러나 **"이색적 순수 비유동알파"가 아니라 잘 알려진 size/유동성 프리미엄**(marcap-clean서 KR_LARGE 대부분 흡수), **(c)** 다만 **저용량 아님 — 유동적 절반서 더 강하고 ADV-weight서 개선 = 고용량·실현가능**(long-illiquid-decile + short-KOSPI200선물, 개별공매도 불필요). 유일 미해결=**상폐포함데이터·PIT멤버십·forward paper**(데이터게이트, 유료/KIS 필요). 이전 "저용량 \$M~수천만"(시점33 impact버그 기반)은 batch3b가 **반박** — 정정.

### 추가(2026-08, 사용자 "수익 너무 적음 → 높은 평균수익 목표로 정보축 포함 모든 가능성 전수 소진"): 시점 35
목표함수 전환: 시장중립 IR(용량제약·modest)이 아니라 **총수익 mean↑**. 이러면 예전 "횡단면서 죽음" 기각들(방향성·집중·이벤트·regime)이 다른 잣대로 재부활. 인프라 정찰(46테이블·뉴스 520만·분기재무·US어닝/공매도·KR insider) → 정보축은 "미개척"이 아니라 대규모 기구축(단 대형ML수프에 IR로만 들어가 씻김·이벤트드리븐 단독 미테스트). 열거 워크플로(6렌즈→18후보→적대비평).
- **BATCH-1/2 실증(`test_earnings_tone/kr_pead/trend_backbone/overnight*.py`)**: ①US PEAD **사망**(초과 −0.04% t=−0.15·size통제 SUE t=−0.05), KR PEAD 노이즈(+1.04% t=1.38·2년 운). ②US 어닝톤 **미미**(초과 +0.39% t=2.66 단조나 US전용·분기1회). ③트렌드/GEM/타이밍 **알파 전부 t<1.1=그냥 베타**, 타이밍이 mean 오히려↓(13.1→9.2% CAGR). ④개별주식 오버나이트 gross Sharpe 2.31이나 **매도세 0.15%(15bp)>gross 12bp = 롱온리 사망**. ⑤KR 공개수급(pykrx 공매도잔고·투자자별·시총) **KRX 로그인월 확정 사망**.
- **★D — 캠페인 최초 realize 관문 통과(오버나이트를 KOSPI200 선물로)**: 개별주식은 세금이 죽였으나 **선물·ETF는 거래세 면제 + 지수 추종.** 검증(`test_overnight_futures/verify/exhaust/us_align.py`, yfinance ^KS200/069500.KS): **캡가중(대형주)서도 살아남**(등가중 아님, ADV가중 Sharpe 2.07·^KS200 Sharpe 1.64). 감쇠 없음(10/12년 양·기간3분할 전부 Sharpe>1·**하락장 2018/2022/2024에도 안 잃음**=진짜 가치). 선물 net(슬리피지+carry) **CAGR +17~22%·Sharpe 1.2~1.5·MDD −22~26%**, buy&hold(0.70) 압도. 인트라데이는 −8.8%(낮에 팔아야).
- **정직 단서(캠페인 철칙 적용)**: (1)+20% 헤드라인은 **역대급 불장(KOSPI200 2025 +90.7%·2026 +73.1%)에 부풀림** → 지속가능 앵커=불장전 2015-2022 **Sharpe ~1.0-1.2·CAGR ~12-15%**. (2)공짜알파 아니라 **밤 갭 리스크(−5~−9% 밤) 보상 프리미엄** — 선물 레버시 갭 대비 사이징 필수. (3)**US↑조건부 Sharpe 7.1은 realize 불가**: 데이터 무결(무결정렬 B=원래 A와 동일, 미래-US 대조군 C corr 0으로 정렬 검증)이나 **경제적 look-ahead** — KR밤(15:30진입)에 US 미개장, US 결과 알 땐 선물야간장이 갭 선반영. corr +0.70은 진짜(KR밤=US 따라잡기 메커니즘) but 조건부 거래불가. (4)요일효과=snooping 의심. (5)ETF(069500) net Sharpe 0.3~0.8(비용에 눌림)→**선물이어야 좋은 net**. (6)**여전히 in-sample — 개장 단일가 실체결·지속성은 forward paper로만.**
- **∴ 시점35**: 무료데이터로 "진짜+실현가능+베타아닌" 고평균 알파는 **오버나이트(무조건부, 선물) 하나뿐**. "돈기계"는 아니나(지속 ~12-15%·Sharpe 1.2·하락장 방어·홈마켓 거래가능) **캠페인 최초의 실현가능 엣지.** 다음=forward paper(KODEX200 ETF via KIS 모의투자 샌드박스로 개장/종가 실체결 확인 → 통과시 선물). 브랜치/문서: 분석하네스 `backend/var/_analysis/test_overnight*.py` 등.
