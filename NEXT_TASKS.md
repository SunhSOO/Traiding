...
> **갱신(2026-07-10, C4v2)**: "US 2025-26 선정 알파 감쇠"는 **재해석됨** — 데이터갭(2024-25) 채워 연속 재학습 walk-forward로 측정하니 **US "감쇠"는 스테일 모델+rank-IC 메트릭 착시**였고, **신선 모델의 top-decile excess는 2024-26에도 유지**(+1.71%, 80% 폴드양수; 단 IC는 여전히 ≈0). ⇒ US는 망가지지 않았고 **주간재학습(ON)이 살림**. **감쇠-대응 모델(레짐/규모 분할) 불필요**(B1 3-seed서 기각). **드리프트 모니터(P0#3)는 rank-IC가 아니라 top-decile excess를 추적해야 함**(IC가 오경보). US+KR **2018-2026 연속캐시 완성** → 최근창 재검증·뉴스 I축 즉시 테스트 인프라 확보.

**올바른 자세**: 알파 사냥 동결(모델 6레버 전부 강건검증서 반증 — swabs_neu·ensemble·mnmh·US/KR평활·B1). 불가피한 페이퍼 축적 대기창 동안 **정직-비용 회계 + 실행 배관 + 안전 모니터 + 킬스위치**를 병렬로 구축. 데이터-게이트 프론티어(뉴스·옵션·PEAD)는 스케줄러가 알아서 익힘.
