...
## 12. 확정된 결정 (2026-06-29 승인)
- **D1 ✅ 타이밍만**: 기술적은 진입 지연+약세 청산만, 바스켓 종목 veto 불가(알파=WHAT 주도).
- **D2 ✅ 신규 `core-kr/us`**: 통합 전략 전용 계정. 기존 ml-/default-는 비교·레거시 잔존.
- **D3 ✅ 레거시 잔존**: composite F/T/I 경로는 비교용 유지.
- **D4 ✅ 바스켓=능동지수 + 노출 오버레이**: v1은 ETF 매수 없이 바스켓+target_exposure로 지수/시황 구현. ETF 인스트루먼트는 향후.
- **D5 ✅ 균등 비중**: target_weight=균등(검증됨). conviction 틸트는 향후 실험.
